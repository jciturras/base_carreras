...
class(of1_a$CODIGO_UNICO)
class(mat1$CODIGO_UNICO)



test <- left_join(x = of1_a,y = mat1,"CODIGO_UNICO") # Tenemos DEMRE y CODIGO_UNICO en la misma base
...
test$DEMRE <- as.numeric(test$DEMRE)




mat15<- mat2015 %>% filter(CODIGO_CARRERA %in% carhomo$DEMRE)  %>% mutate(PUNTAJE_PONDERADO=PUNTAJE_PONDERADO/100)
mat15a<- mat15 %>% group_by(CODIGO_CARRERA) %>% summarise(MIN_PSU=min(PUNTAJE_PONDERADO),
                                                         MEDIAN_PSU=median(PUNTAJE_PONDERADO),
                                                         MEAN_PSU=mean(PUNTAJE_PONDERADO),
                                                         SD_PSU=sd(PUNTAJE_PONDERADO),
                                                         PER10_PSU=quantile(PUNTAJE_PONDERADO, probs = c(.10)),
                                                         PER25_PSU=quantile(PUNTAJE_PONDERADO, probs = c(.25)),
                                                         PER50_PSU=quantile(PUNTAJE_PONDERADO, probs = c(.50)),
                                                         PER75_PSU=quantile(PUNTAJE_PONDERADO, probs = c(.75)),
                                                         PER90_PSU=quantile(PUNTAJE_PONDERADO, probs = c(.90))) %>% rename(DEMRE=CODIGO_CARRERA)

# Juntar las 3 bases
base1 <- left_join(x = carhomo, y = mat15a,"DEMRE") # Tenemos DEMRE y CODIGO_UNICO en la misma base
base1 <- base1 %>% select(DEMRE, NOMBRE_CARRERA,CARRERA_HOMOLOGADA,everything())
base1 <- base1  %>% select(-c("AREA_CARRERA_GENERICA","NOMBRE_CARRERA","NOMBRE_IES","ARANCEL_ANUAL","PUNTAJE_CORTE","TIPO_IES","CODIGO_IES","AREA_CONOCIMIENTO","OECD_AREA","OECD_SUBAREA"))
names(base1)

base2 <- left_join(x = test,y = base1,"DEMRE")
base2 <- base2 %>% select(ID_CARRERA ,DEMRE, NOMBRE_CARRERA,CARRERA_HOMOLOGADA,everything(),-c("PEDAGOGIA_MEDICINA_OTRO","REQUISITO_INGRESO","SEMESTRES_RECONOCIDOS","NUMERO_RAMOS_REQUERIDO_EGRESO","AREA_ACTUAL","AREA_DEST_AGRICULTURA",
                                                                                                "AREA_DEST_CIENCIAS","AREA_DEST_C_SOC_COMERC_DERECHO","AREA_DEST_EDUCACION","AREA_DEST_HUMANIDADES_Y_ARTES","AREA_DEST_ING_IND_CONST","AREA_DEST_SALUD_Y_SERV_SOC",
                                                                                                "AREA_DEST_SERVICIOS"))
names(base2)
...
base2 <- base2 %>% mutate(PROPORCION_MUJERES=MATRICULADOS_MUJERES_POR_CARRERA/TOTAL_MATRICULADOS,
                          PROPORCION_HOMBRES=MATRICULADOS_HOMBRES_POR_CARRERA/TOTAL_MATRICULADOS)


xlsx::write.xlsx(as.data.frame(base2), file = "carreras_info.xlsx")
